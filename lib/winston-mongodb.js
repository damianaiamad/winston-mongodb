 ...
 * suppress output.
 * @param {string} options.db MongoDB connection uri or preconnected db
...
let MongoDB = exports.MongoDB = function(options) {
  ...
  options = (options || {});
  if (!options.db) {
    throw new Error('You must provide db uri to log to.');
  }
  if (!options.dbName) {
    throw new Error('You should provide db name to log to.');
  }
  ...
  function setupDatabaseAndEmptyQueue(mongoClient) {
    let db = mongoClient.db(self.dbName)
